...
from ..extre_module.custom_nn.upsample.WaveletUnPool import WaveletUnPool
from ..extre_module.custom_nn.downsample.WaveletPool import WaveletPool
...
class HybridEncoder(nn.Module):     
    ...
    def __init__(self,     
        ...
        for _ in range(len(in_channels) - 1, 0, -1):  # 从高层到低层遍历     
            ...
            # 小波变换上采样池化
            self.fpn_upsample_blocks.append(
                nn.Sequential(WaveletUnPool(), ConvNormLayer_fuse(hidden_dim // 4, hidden_dim, 1, 1))
            )
        ...
        for _ in range(len(in_channels) - 1):    # 从低层到高层遍历
            ...
            # 小波变化下采样
            self.downsample_convs.append(  
                nn.Sequential(WaveletPool(), ConvNormLayer_fuse(hidden_dim*4, hidden_dim, 1, 1))
                if version == 'dfine' else ConvNormLayer_fuse(hidden_dim, hidden_dim, 3, 2, act=act)  
            )  
